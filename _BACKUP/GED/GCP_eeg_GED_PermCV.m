...
run_scope = 'full'; % 'full' or 'perm_check'
...
min_eigval_hard = 1;                % hard minimum GED eigenvalue
...
perm_valid_null_target = 300;        % adaptive target: valid hard-gated null draws per subject
perm_valid_null_min = 100;           % minimum valid hard-gated null draws required for stable threshold
n_perm_max = 20000;                  % maximum permutation attempts per subject
perm_alpha = 0.5;                    % exploratory sensitivity setting
perm_multiple_testing = 'bh_fdr';    % component-wise empirical p-values with BH-FDR per subject
...
if ispc
    fig_save_dir_benchmark = 'W:\Students\Arne\GCP\figures\eeg\ged\benchmark';
    fig_save_dir_subj = 'W:\Students\Arne\GCP\figures\eeg\ged\subj';
else
    fig_save_dir_benchmark = '/Volumes/g_psyplafor_methlab$/Students/Arne/GCP/figures/eeg/ged/benchmark';
    fig_save_dir_subj = '/Volumes/g_psyplafor_methlab$/Students/Arne/GCP/figures/eeg/ged/subj';
end
if ~exist(fig_save_dir_benchmark, 'dir'), mkdir(fig_save_dir_benchmark); end
if ~exist(fig_save_dir_subj, 'dir'), mkdir(fig_save_dir_subj); end
comp_sel_save_dir = fullfile(fig_save_dir_subj, 'component_selection');
...
all_perm_fdr_q = nan(1, nSubj);
all_perm_null_eigs = nan(1, nSubj);
all_postperm_fallback_used = false(1, nSubj);
all_postperm_fallback_reason = repmat({''}, 1, nSubj);
all_postperm_candidate_counts = nan(1, nSubj);
all_postperm_selected_counts = nan(1, nSubj);
all_postperm_perm_pass_counts = nan(1, nSubj);
...
for subj = 1:nSubj
    ...
    if exclude_dominant_outlier
        ...
        if ~isempty(dominant_outlier_idx)
            ...
            msg = sprintf(['Dominant top GED component excluded for subject %s (component C%d) ', ...
                           'before permutation/candidate selection.'], subjects{subj}, dominant_outlier_idx);
            warning(msg);
            ratio12_val = NaN;
    ...
    perm_threshold = NaN;
    perm_null_max = nan(n_perm_max, 1);
    perm_null_eigs = nan(n_perm_max * nSearch, 1);
    n_perm_null_eigs = 0;
    n_perm_attempted = 0;
    n_perm_valid_hard_null = 0;
    perm_unstable_hard_null = false;
    perm_sig_mask = true(nSearch, 1);
    perm_pvals = nan(nSearch, 1);
    perm_qvals = nan(nSearch, 1);
    postPermCV_fallback_used = false;
    postPermCV_fallback_reason = '';
    if strcmpi(comp_select_mode, 'perm_cv')
        ...
        if nStimTrials > 0 && nBaseTrials > 0
            ...
            while n_perm_attempted < n_perm_max && n_perm_valid_hard_null < perm_valid_null_target
                n_batch = min(n_perm, n_perm_max - n_perm_attempted);
                for bi = 1:n_batch
                    n_perm_attempted = n_perm_attempted + 1;
                    pidx = randperm(nPool);
                    ...
                    hard_perm_for_null = hard_perm;
                    if exclude_dominant_outlier
                        [hard_perm_for_null, ~] = exclude_dominant_top_outlier( ...
                            eval_perm, hard_perm, outlier_ratio_thr, outlier_mad_mult, outlier_min_rest);
                    end
                    if any(hard_perm_for_null)
                        % Count permutation draws with >=1 valid hard-gated component.
                        n_perm_valid_hard_null = n_perm_valid_hard_null + 1;
                        perm_null_max(n_perm_valid_hard_null) = max(eval_perm(hard_perm_for_null));
                        eig_vals_i = eval_perm(hard_perm_for_null);
                        n_add = numel(eig_vals_i);
                        if n_add > 0
                            perm_null_eigs(n_perm_null_eigs + (1:n_add)) = eig_vals_i;
                            n_perm_null_eigs = n_perm_null_eigs + n_add;
                        end
                    end
                end
            end
            valid_null = perm_null_max(1:n_perm_valid_hard_null);
            if n_perm_valid_hard_null >= perm_valid_null_min
                perm_threshold = prctile(valid_null, 100*(1-perm_alpha));
                null_eigs = perm_null_eigs(1:n_perm_null_eigs);
                hard_idx = find(hard_eligible);
                if ~isempty(hard_idx) && ~isempty(null_eigs)
                    denom_null = 1 + n_perm_null_eigs;
                    for hi = 1:numel(hard_idx)
                        ci = hard_idx(hi);
                        perm_pvals(ci) = (1 + sum(null_eigs >= evals_sorted(ci))) / denom_null;
                    end
                    q_hard = bh_fdr_adjust(perm_pvals(hard_idx));
                    perm_qvals(hard_idx) = q_hard;
                    perm_sig_mask = false(nSearch, 1);
                    perm_sig_mask(hard_idx) = q_hard <= perm_alpha;
                else
                    perm_sig_mask = false(nSearch, 1);
                end
                if n_perm_valid_hard_null < perm_valid_null_target
                    msg = sprintf(['Permutation valid hard-null draws below target for subject %s ', ...
                                   '(valid=%d, target=%d, attempted=%d, max=%d).'], ...
                                  subjects{subj}, n_perm_valid_hard_null, perm_valid_null_target, ...
                                  n_perm_attempted, n_perm_max);
                    warning(msg);
                    warning_log = append_subject_warning(warning_log, subjects{subj}, 'PERM_VALID_DRAWS_BELOW_TARGET', msg, ...
                        struct('n_valid_null', n_perm_valid_hard_null, 'target_valid_null', perm_valid_null_target, ...
                               'min_required', perm_valid_null_min, 'n_perm_attempted', n_perm_attempted, ...
                               'n_perm_batch', n_perm, 'n_perm_max', n_perm_max, 'perm_alpha', perm_alpha));
                end
            else
                perm_unstable_hard_null = true;
                msg = sprintf(['Insufficient valid hard-gated null draws for subject %s ', ...
                               '(valid=%d, min=%d, attempted=%d, max=%d). ', ...
                               'Permutation gating disabled for this subject.'], ...
                              subjects{subj}, n_perm_valid_hard_null, perm_valid_null_min, ...
                              n_perm_attempted, n_perm_max);
                warning(msg);
                warning_log = append_subject_warning(warning_log, subjects{subj}, 'PERM_UNSTABLE_HARD_NULL', msg, ...
                    struct('n_valid_null', n_perm_valid_hard_null, 'min_required', perm_valid_null_min, ...
                           'target_valid_null', perm_valid_null_target, 'n_perm_attempted', n_perm_attempted, ...
                           'n_perm_batch', n_perm, 'n_perm_max', n_perm_max, 'perm_alpha', perm_alpha));
                perm_sig_mask = hard_eligible;
            end
        else
            msg = sprintf('Permutation testing skipped for subject %s due to insufficient trials.', subjects{subj});
            warning(msg);
            warning_log = append_subject_warning(warning_log, subjects{subj}, 'PERM_SKIPPED_INSUFFICIENT_TRIALS', msg, ...
                struct('n_stim_trials', nStimTrials, 'n_base_trials', nBaseTrials, 'n_pool_trials', nPool));
            perm_sig_mask = false(nSearch, 1);
        end
    end
    all_perm_thresholds(subj) = perm_threshold;
    all_perm_fdr_q(subj) = perm_alpha;
    all_perm_null_eigs(subj) = n_perm_null_eigs;
    candidate_table.perm_sig = perm_sig_mask;
    candidate_table.perm_p = perm_pvals;
    candidate_table.perm_q = perm_qvals;
    ...
    if isempty(candidate_pool_idx)
        msg = sprintf(['No components survived hard+permutation gating for subject %s. ', ...
                       'Falling back to single best ranked component.'], subjects{subj});
        warning(msg);
        warning_log = append_subject_warning(warning_log, subjects{subj}, 'NO_POSTPERM_COMPONENTS', msg, ...
            struct('n_hard_eligible', sum(hard_eligible), 'n_perm_sig', sum(perm_sig_mask), ...
                   'perm_threshold', perm_threshold, 'perm_fdr_q', perm_alpha, ...
                   'perm_method', perm_multiple_testing));
        candidate_pool_idx = bestIdx;
        postPermCV_fallback_used = true;
        postPermCV_fallback_reason = 'No components survived hard+permutation gating';
    else
    ...
    if k_max < 1
        ...
        candidate_pool_idx = bestIdx;
        postPermCV_fallback_used = true;
        if isempty(postPermCV_fallback_reason)
            postPermCV_fallback_reason = 'Empty candidate pool after selection';
        end
    end
    ...
    if isempty(selected_idx)
        ...
        postPermCV_fallback_used = true;
        if isempty(postPermCV_fallback_reason)
            postPermCV_fallback_reason = 'No selected components after CV; using best ranked component';
        end
    end
    ...
    if isempty(preperm_idx)
        msg = sprintf(['No hard-eligible pre-Perm+CV components for subject %s. ', ...
                       'Falling back to best single component for pre-Perm+CV combined benchmark.'], subjects{subj});
        warning(msg);
        warning_log = append_subject_warning(warning_log, subjects{subj}, 'NO_PREPERM_COMPONENTS', msg, ...
    ...
    all_component_selection_stats{subj} = struct( ...
        ...
        'perm_multiple_testing', perm_multiple_testing, ...
        'perm_unstable_hard_null', perm_unstable_hard_null, ...
        'perm_alpha', perm_alpha, ...
        'n_perm', n_perm, ...
        'perm_valid_null_target', perm_valid_null_target, ...
        'perm_valid_null_min', perm_valid_null_min, ...
        'n_perm_max', n_perm_max, ...
        'n_perm_attempted', n_perm_attempted, ...
        'n_perm_valid_hard_null', n_perm_valid_hard_null, ...
        'n_perm_null_eigs', n_perm_null_eigs, ...
        'n_cv_rows', nCvRows, ...
        'cv_folds', cv_folds, ...
        ...
        'postperm_fallback_used', postPermCV_fallback_used, ...
        'postperm_fallback_reason', postPermCV_fallback_reason);
    all_postperm_fallback_used(subj) = postPermCV_fallback_used;
    all_postperm_fallback_reason{subj} = postPermCV_fallback_reason;
    all_postperm_candidate_counts(subj) = numel(candidate_pool_idx);
    all_postperm_selected_counts(subj) = numel(selected_idx);
    all_postperm_perm_pass_counts(subj) = sum(hard_eligible & perm_sig_mask);
    ...
    if no_hard_threshold_match
        annotation(fig_sel, 'textbox', [0.01 0.93 0.98 0.03], ...
            'String', 'FALLBACK COMPONENT USED: No component met hard thresholds (pre-Perm+CV)', ...
            'EdgeColor', [0.7 0 0], 'Color', [0.7 0 0], ...
    ...
    annotation(fig_post, 'textbox', [0.01 0.965 0.98 0.03], ...
        ...
        'VerticalAlignment', 'middle', 'FontSize', 14, 'FontWeight', 'bold');
    if postPermCV_fallback_used
        if isempty(postPermCV_fallback_reason)
            postPermCV_fallback_reason = 'Fallback component used';
        end
        annotation(fig_post, 'textbox', [0.01 0.93 0.98 0.03], ...
            'String', ['FALLBACK COMPONENT USED: ', postPermCV_fallback_reason], ...
            'EdgeColor', [0.7 0 0], 'Color', [0.7 0 0], ...
            'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
            'FontSize', 11, 'FontWeight', 'bold', ...
            'BackgroundColor', [1 0.94 0.94]);
    ...
    if strcmpi(run_scope, 'perm_check')
        fprintf('Subject %s: perm-check mode active, Phase 2 skipped.\n', subjects{subj});
        toc
        continue;
    end
    ...
    if isempty(W_sel)
        msg = sprintf('No selected filters for subject %s. Falling back to best single component.', subjects{subj});
        warning(msg);
        warning_log = append_subject_warning(warning_log, subjects{subj}, 'EMPTY_W_SEL', msg, struct());
        W_sel = zeros(0, 0);
    end
    if isempty(W_pre)
        msg = sprintf('No pre-Perm+CV filters for subject %s. Falling back to best single component.', subjects{subj});
        warning(msg);
        warning_log = append_subject_warning(warning_log, subjects{subj}, 'EMPTY_W_PRE', msg, struct());
...
if strcmpi(run_scope, 'perm_check')
    nFallback = sum(all_postperm_fallback_used);
    fprintf('\n============================================================\n');
    fprintf('Permutation-check summary (Phase 1 only)\n');
    fprintf('============================================================\n');
    fprintf('Subjects with fallback: %d/%d\n', nFallback, nSubj);
    for subj = 1:nSubj
        reason_i = all_postperm_fallback_reason{subj};
        if isempty(reason_i)
            reason_i = 'none';
        end
        fprintf('  %s: fallback=%d, permQ=%.3f, permPass=%d, kSel=%d, reason=%s\n', ...
            subjects{subj}, all_postperm_fallback_used(subj), all_perm_fdr_q(subj), ...
            all_postperm_perm_pass_counts(subj), all_postperm_selected_counts(subj), reason_i);
    end

    if ispc
        save_path_perm = 'W:\Students\Arne\GCP\data\features\GCP_eeg_GED_permcheck.mat';
    else
        save_path_perm = '/Volumes/g_psyplafor_methlab$/Students/Arne/GCP/data/features/GCP_eeg_GED_permcheck.mat';
    end
    save(save_path_perm, ...
        'subjects', 'run_scope', 'all_perm_thresholds', ...
        'all_perm_fdr_q', 'all_perm_null_eigs', ...
        'all_postperm_fallback_used', 'all_postperm_fallback_reason', ...
        'all_postperm_candidate_counts', 'all_postperm_selected_counts', ...
        'all_postperm_perm_pass_counts', 'all_component_selection_stats', ...
        'all_cv_curves', 'warning_log', 'all_threshold_inspection', ...
        'threshold_inspection_targets');
    fprintf('Saved permutation-check output: %s\n', save_path_perm);
    fprintf('Done (perm_check mode).\n');
    return;
end
...
for subj = 1:nSubj
    ...
    saveas(fig_bench_subj, fullfile(fig_save_dir_benchmark, sprintf('GCP_eeg_GED_benchmark_subj%s.png', subjects{subj})));
end
...
saveas(fig_cent, fullfile(fig_save_dir_benchmark, 'GCP_eeg_GED_centroid_summary.png'));
...
saveas(fig_bench_group, fullfile(fig_save_dir_benchmark, 'GCP_eeg_GED_benchmark_grandaverage.png'));
...
saveas(fig_cond_slope, fullfile(fig_save_dir_benchmark, 'GCP_eeg_GED_condition_slope.png'));
...
for cond = 1:4
    ...
    if any(isfinite(mu))
        ...
        % Power spectra with edge-reflective smoothing.
        mu_plot = smooth_reflective(mu, 5);
...
saveas(fig_grand_psd, fullfile(fig_save_dir_benchmark, 'GCP_eeg_GED_grand_average_power_spectrum.png'));
...
saveas(fig_all, fullfile(fig_save_dir_benchmark, 'GCP_eeg_GED_all_subjects.png'));
...
saveas(fig_det, fullfile(fig_save_dir_benchmark, 'GCP_eeg_GED_detection_rate.png'));
...
save(save_path, ...
    ...
    'all_postperm_fallback_used', 'all_postperm_fallback_reason', ...
    'all_postperm_candidate_counts', 'all_postperm_selected_counts', ...
    'all_postperm_perm_pass_counts', 'warning_log', ...
...
for wi = 1:numel(warning_log)
    ...
    elseif strcmp(w.code, 'NO_POSTPERM_COMPONENTS')
        m = w.metrics;
        fprintf(['     post-perm diagnostics: nHardEligible=%d, nPermSig=%d, ', ...
                 'permMethod=%s, q=%.3f\n'], ...
            m.n_hard_eligible, m.n_perm_sig, m.perm_method, m.perm_fdr_q);
    elseif strcmp(w.code, 'PERM_TOO_FEW_VALID_DRAWS')
        m = w.metrics;
        fprintf('     permutation diagnostics: validNull=%d, minRequired=%d, nPerm=%d, alpha=%.3f\n', ...
            m.n_valid_null, m.min_required, m.n_perm, m.perm_alpha);
    elseif strcmp(w.code, 'PERM_VALID_DRAWS_BELOW_TARGET')
        m = w.metrics;
        fprintf(['     permutation diagnostics: validNull=%d, target=%d, minRequired=%d, ', ...
                 'attempted=%d, batch=%d, max=%d, alpha=%.3f\n'], ...
            m.n_valid_null, m.target_valid_null, m.min_required, m.n_perm_attempted, ...
            m.n_perm_batch, m.n_perm_max, m.perm_alpha);
    elseif strcmp(w.code, 'PERM_UNSTABLE_HARD_NULL')
        m = w.metrics;
        fprintf(['     permutation diagnostics: validNull=%d, minRequired=%d, target=%d, ', ...
                 'attempted=%d, batch=%d, max=%d, alpha=%.3f; action=perm-gating-disabled\n'], ...
            m.n_valid_null, m.min_required, m.target_valid_null, m.n_perm_attempted, ...
            m.n_perm_batch, m.n_perm_max, m.perm_alpha);
    elseif strcmp(w.code, 'PERM_SKIPPED_INSUFFICIENT_TRIALS')
        m = w.metrics;
        fprintf('     trial diagnostics: nStim=%d, nBase=%d, nPool=%d\n', ...
            m.n_stim_trials, m.n_base_trials, m.n_pool_trials);
