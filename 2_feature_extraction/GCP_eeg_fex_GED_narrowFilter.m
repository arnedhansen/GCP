...
% TEMP TEST COPY: narrowband GED training, broadband spectral application.
% All figures and result .mat files are written under:
%   /Volumes/g_psyplafor_methlab$/Students/Arne/GCP/figures/tests/GED_narrowFilter
%
% Narrowband GED and component selection (per subject)
%   - Pool trials across conditions and compute gamma-band covariances
%      (45-75 Hz FIR) for three windows (full/early/late) and baseline.
...
%   - Project each trial (unfiltered sensor data) to the combined GED
%      component space and compute spectrum-based power scans on a
%      30-90 Hz grid in dB.
...
analysis_freq_range = [30 90]; % spectral scanning / peak detection band
ged_train_freq_range = [45 75]; % FIR band used only for GED covariances
...
% Input EEG still comes from the project features tree; all outputs go to the test folder
...
out_root = '/Volumes/g_psyplafor_methlab$/Students/Arne/GCP/figures/tests/GED_narrowFilter';
fig_save_dir_ged = out_root;
...
for subj = 1:nSubj
    ...
    clc; close all; fprintf('[GED] Subject %s (%d/%d) narrowtrain [%d-%d Hz] apply [%d-%d Hz] (%d occ channels)\n', ...
        subjects{subj}, subj, nSubj, ged_train_freq_range(1), ged_train_freq_range(2), ...
        analysis_freq_range(1), analysis_freq_range(2), nOcc);
    ...
    for cond = 1:4
        ...
        cfg_filt.bpfreq     = ged_train_freq_range;
        cfg_filt.bpfilttype = 'fir';
        cfg_filt.bpfiltord  = round(3 * fsample / ged_train_freq_range(1));
...
%% Save results
save_path = fullfile(out_root, 'GCP_eeg_GED_narrowFilter.mat');
save(save_path, ...
    'trials_powratio', ...
    ...
    'analysis_freq_range', 'ged_train_freq_range', ...
    'baseline_window', 'full_window', 'early_window', 'late_window');

clc
powspctrm_save_path = fullfile(out_root, 'GCP_eeg_powspctrm_GED_narrowFilter.mat');
...
save(fullfile(out_root, 'GCP_subject_inclusion_narrowFilter.mat'), 'subject_inclusion', '-v7.3');

fprintf('[GED] DONE!\n');
fprintf('[GED] Feature extraction results saved to: %s\n', save_path);
fprintf('[GED] Figures and diagnostics saved under: %s\n', out_root);
for si = 1:nSubj
