## Simulates SESOI-based power for gamma power with mixed models and saves a CSV power curve plus PNG figure
...
result <- runSESOI()
print(result)
## Simulates SESOI-based power for gamma power with mixed models
...
runSESOI <- function() {
  ...
  seed <- 124L # Set deterministic seed to make simulation output reproducible
  ...
  sim_col <- "gamma_power" # Define simulated dependent-variable column name
  target_term <- "contrast_num_c2" # Define fixed effect term tested for significance
  linear_nuisance_beta <- 0.05 # Keep linear contrast effect as nuisance while testing quadratic SESOI
  model_formula <- gamma_power ~ contrast_num_c + contrast_num_c2 + (1 + contrast_num_c | Subject) # Define fitted LMM for each synthetic dataset
  ...
  output_prefix <- "GCP_power_analysis_gamma_power_SESOI_only" # Use one explicit filename stem for all result files
  plot_title <- "Power Curve: Gamma Power SESOI (contrast_num_c2)" # Use one explicit title for figure output
  ...
  scenario <- data.frame( # Define SESOI-generating parameters as a one-row data frame
    ...
    beta_raw = 0.05, # Set SESOI effect size on quadratic predictor
    ...
    random_slope_sd = 0.10, # Set between-subject linear-slope variability
  ...
  estimate_power_chunk <- function(
    ...
    for (i in seq_len(chunk_nsim)) { # Repeat simulation-fit-test cycle chunk_nsim times
      ...
      contrast_num_c <- as.numeric(scale(contrast_num, center = TRUE, scale = TRUE)) # Z-scale linear predictor to stabilize model fitting
      contrast_num_c2 <- contrast_num_c^2 # Build quadratic predictor used as tested SESOI effect
      dat <- data.frame(Subject = Subject, contrast_num_c = contrast_num_c, contrast_num_c2 = contrast_num_c2) # Build model-data frame structure
      ...
      beta_raw <- as.numeric(scenario$beta_raw[1]) # Extract true quadratic fixed effect
      ...
      random_slopes <- rnorm(n_subject_levels, mean = 0, sd = rs_sd * rs_mult) # Draw subject-specific linear-slope deviations
      x <- dat$contrast_num_c # Reference linear predictor once for readability
      x2 <- dat$contrast_num_c2 # Reference quadratic predictor once for readability
      mu <- mu0 + random_intercepts[dat$Subject] + random_slopes[dat$Subject] * x + linear_nuisance_beta * x + beta_raw * x2 # Construct expected value per observation
