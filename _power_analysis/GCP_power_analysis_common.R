...
estimate_power_chunk <- function(
  ...
  for (i in seq_len(chunk_nsim)) {
    ...
    dat <- simulate_outcome(dat, scenario, outcome_cfg)
    fit_out <- fit_model_and_extract(dat, outcome_cfg, alpha)
    rejects <- rejects + as.integer(fit_out$reject)
    singular_hits <- singular_hits + as.integer(fit_out$singular)
    w_n <- length(fit_out$warnings)
    warning_hits <- warning_hits + as.integer(w_n > 0)
    warning_total <- warning_total + w_n
  }

  list(
    chunk_nsim = chunk_nsim,
    rejects = rejects,
    singular_hits = singular_hits,
...
estimate_power_parallel <- function(
  ...
  log_progress("Start: ", context_label, " | nsim=", nsim, " | parallel_workers=", n_workers, verbose = verbose)

  round_chunk_nsim <- max(1L, as.integer(round_chunk_nsim))
  total_nsim <- 0L
  total_rejects <- 0L
  total_singular <- 0L
  total_warning_hits <- 0L
  total_warning <- 0L
  ...
  while (remaining > 0L) {
    ...
    chunk_results <- parallel::parLapply(
      cl,
      chunk_sizes,
      fun = function(chunk_nsim, scenario, outcome_cfg, n_subjects, trials_per_condition, alpha) {
    ...
    total_singular <- total_singular + sum(vapply(chunk_results, `[[`, integer(1), "singular_hits"))
    total_warning_hits <- total_warning_hits + sum(vapply(chunk_results, `[[`, integer(1), "warning_hits"))
    total_warning <- total_warning + sum(vapply(chunk_results, `[[`, integer(1), "warning_total"))
    remaining <- nsim - total_nsim

    elapsed <- as.numeric(difftime(Sys.time(), t0, units = "mins"))
    interim_power <- total_rejects / total_nsim
    log_progress(
      "Heartbeat: ", context_label,
      " | round=", round_idx,
      " | processed=", total_nsim, "/", nsim,
      " | interim_power=", sprintf("%.3f", interim_power),
      " | elapsed_min=", sprintf("%.1f", elapsed),
      verbose = verbose
    )
  }
  ...
  out <- data.frame(
    ...
    mc_se = se,
    nsim = total_nsim,
    singular_rate = total_singular / total_nsim,
    warning_rate = total_warning_hits / total_nsim,
    warnings_per_fit = total_warning / total_nsim
  )
  log_progress(
    "Done: ", context_label,
    " | power=", sprintf("%.3f", out$power),
...
run_outcome_power_analysis <- function(outcome_cfg) {
  ...
  parallel_round_chunk_nsim <- if (!is.null(outcome_cfg$parallel_round_chunk_nsim)) as.integer(outcome_cfg$parallel_round_chunk_nsim) else 2L
  ...
  for (i in seq_len(nrow(scenario_manifest))) {
    ...
    if (use_parallel) {
      scenario_rows <- vector("list", length(subject_breaks))
      for (n_idx in seq_along(subject_breaks)) {
        n <- subject_breaks[n_idx]
        scenario_rows[[n_idx]] <- estimate_power_parallel(
          scenario = sc,
          outcome_cfg = outcome_cfg,
          n_subjects = n,
          nsim = nsim,
          ...
          round_chunk_nsim = parallel_round_chunk_nsim
        )
        ## Runtime checkpoint written after each completed sample-size point.
        checkpoint_rows <- scenario_rows[seq_len(n_idx)]
        checkpoint_rows <- checkpoint_rows[!vapply(checkpoint_rows, is.null, logical(1))]
        if (length(checkpoint_rows) > 0) {
          checkpoint_df <- do.call(rbind, checkpoint_rows)
          write.csv(
            checkpoint_df,
            file.path(output_dir, paste0(outcome_cfg$file_prefix, "_checkpoint_", sc$scenario_label, ".csv")),
            row.names = FALSE
          )
        }
      }
