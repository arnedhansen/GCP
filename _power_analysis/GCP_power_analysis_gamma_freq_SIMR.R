...
## Effect assumptions for frequency linear trend:
## - Uses plausible small-to-moderate effects without over-anchoring to pilot.
effect_scenarios <- data.frame(
  effect_label = c("SESOI", "Conservative", "Optimistic"),
  beta_linear = c(0.20, 0.35, 0.50)
)
...
run_scenario <- function(beta_linear, random_slope_sd) {
  dat <- make_design(N_SUBJECT_BASE, TRIALS_PER_CONDITION)

  ## Build synthetic baseline values and then overwrite target parameters.
  subject_intercepts <- rnorm(nlevels(dat$Subject), 0, 0.60)
  subject_slopes <- rnorm(nlevels(dat$Subject), 0, random_slope_sd)
  linear_x <- dat$contrast_num_c
  dat$gamma_frequency <- 55 +
    subject_intercepts[dat$Subject] +
    subject_slopes[dat$Subject] * linear_x +
    beta_linear * linear_x +
    rnorm(nrow(dat), 0, 3.00)
  ...
  primary_model <- suppressWarnings(
    lmer(gamma_frequency ~ contrast_num_c + (1 | Subject), data = dat, REML = FALSE)
  )
  slope_model <- suppressWarnings(
    lmer(gamma_frequency ~ contrast_num_c + (1 + contrast_num_c | Subject), data = dat, REML = FALSE)
  )
  ...
  ## Explicit inferential target aligns with H2 monotonic trend.
  test_term <- "contrast_num_c"
...
for (i in seq_len(nrow(scenario_grid))) {
  ...
  beta_linear <- effect_scenarios$beta_linear[e_idx]
  ...
  sim_out <- run_scenario(beta_linear = beta_linear, random_slope_sd = slope_sd)
  results[[i]] <- sim_out

  ps <- summary(sim_out$power_sim)
  summary_rows[[i]] <- data.frame(
    outcome = "gamma_frequency",
...
write.csv(
  scenario_summary,
  file = file.path(output_dir, "GCP_power_analysis_gamma_frequency_summary.csv"),
  row.names = FALSE
)
write.csv(
  power_curve_df,
  file = file.path(output_dir, "GCP_power_analysis_gamma_frequency_curve.csv"),
  row.names = FALSE
)
saveRDS(
  results,
  file = file.path(output_dir, "GCP_power_analysis_gamma_frequency_results.rds")
)

## Plot power curves across sensitivity scenarios.
png(
  file = file.path(output_dir, "GCP_power_analysis_gamma_frequency.png"),
...
ggplot(
  ...
  labs(
    ...
    title = "SIMR Power Curves: Gamma Frequency (contrast_num_c)"
  ) +
