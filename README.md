...
`GCP_preprocessing.m` segments merged EEG+ET data into epochs around stimulus onset ([-2, 3.5] s), converts to FieldTrip format, and writes per-condition EEG and ET files. Baseline for gaze metric extraction: [-1.5, -0.25] s; analysis window for scalar gaze metrics: [0.3, 2] s. EyeLink event counts (blinks, fixations, saccades) are extracted per trial during preprocessing. Outputs go to `data/features/<subject>/eeg` (`dataEEG.mat`) and `.../gaze` (`dataET.mat`, `gaze_metrics.mat`).
